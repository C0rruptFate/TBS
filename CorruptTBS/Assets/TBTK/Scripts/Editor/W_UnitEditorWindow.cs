	...
	public class NewUnitEditorWindow : TBEditorWindow {
		...
		protected float DrawUnitStats(float startX, float startY, Unit unit){
			...
			if(foldStats){
				...
				cont=new GUIContent("Attack Range:", "Attack range in term of tile");
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				unit.attackRange=EditorGUI.IntField(new Rect(startX+spaceX, startY, widthS, height), unit.attackRange);
