	...
	public class NewUnitAbilityEditorWindow : NewAbilityEditorWindow {
		...
		protected float DrawTargetingSetting(float startX, float startY, UnitAbility ability){
			...
			if(foldTargeting){
				...
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				if(ability.requireTargetSelection && ability.type!=UnitAbility._AbilityType.Overwatch){
				...
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				if(ability.requireTargetSelection) ability.range=EditorGUI.IntField(new Rect(startX+spaceX, startY, widthS, height), ability.range);
				else EditorGUI.LabelField(new Rect(startX+spaceX, startY, widthS, height), "-");
				...
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				if(ability.requireTargetSelection) ability.normalAttack=EditorGUI.Toggle(new Rect(startX+spaceX, startY, widthS, height), ability.normalAttack);
				else EditorGUI.LabelField(new Rect(startX+spaceX, startY, widthS, height), "-");
				...
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				if(!ability.requireTargetSelection || !ability.normalAttack) EditorGUI.LabelField(new Rect(startX+spaceX, startY, widthS, height), "-");
				else ability.requireDirectLOS=EditorGUI.Toggle(new Rect(startX+spaceX, startY, widthS, height), ability.requireDirectLOS);
				
				cont=new GUIContent(" - ShootObject:", "The shoot object to use if the ability involve shooting");
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				if(!ability.requireTargetSelection || !ability.normalAttack) EditorGUI.LabelField(new Rect(startX+spaceX, startY, widthS, height), "-");
				else ability.shootObject=(GameObject)EditorGUI.ObjectField(new Rect(startX+spaceX, startY, width-15, height), ability.shootObject, typeof(GameObject), false);
